export async function fetchOpengatewayModels() {
  try {
    const response = await fetch('/api/opengateway/models');
    if (!response.ok) throw new Error("Failed to fetch Opengateway models");
    return await response.json();
  } catch (error) {
    console.error("Opengateway Models fetch error", error);
    return [
      "mimo-v2.5-pro",
      "mimo-v2-pro",
      "mimo-v2.5",
...
export async function generateOpengatewayChatResponse(
  model: string,
  systemPrompt: string,
  ...
  const response = await fetch('/api/opengateway/chat', {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(err.error || "Failed to generate Opengateway response");
  }
