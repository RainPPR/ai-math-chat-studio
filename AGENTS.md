...
- 配置模型参数（temperature、maxTokens、reasoningEffort、thinkingLevel 等）。
  ...
  - **自动试错机制**：当 `reasoningEffort` 未指定时，流式生成器会采用级联回退机制，依次尝试以 `max` -> `xhigh` -> `high` 强度调用（每个层级最多重试一次），若均失败，最终退回无 `reasoning_effort` 参数调用。特别地，若检测到请求过快或速率限制 (429/rate-limit)，为保证效率不进行后面的尝试，直接向用户抛出错误。
