...
2. **Provider/Model 两级配置**：用户先配置提供商实例，再配置绑定到提供商的模型实例
3. **3 种内置类型**：Google Gemini、Nvidia NIM、OpenAI Compatible
4. **统一 API 客户端**：`src/lib/api.ts` 替代了所有旧的供应商客户端文件
5. **模块化后端**：`server/` 目录分离 providers、built-in、routes、services
6. **SSE 订阅**：前端通过 `EventSource` 订阅生成进度，支持断线重连
...
6. **两级架构**：Providers（配置连接信息）+ Models（配置模型参数），解耦后可复用提供商
