...
### ModelPoolEntry（模型池条目）
```typescript
interface ModelPoolEntry {
  id: string;                       // UUID
  providerId: string;               // 供应商 ID（nvidia/cloudflare/aihubmix/opengateway/poe/gemini）
  modelId: string;                  // 模型 ID
  displayName?: string;             // 显示名称（可选）
  baseURL?: string;                 // 自定义 Base URL（覆盖提供商默认值）
  ...
  thinkingLevel?: string;           // 思考级别（Gemini 专用：none/minimal/low/medium/high）
...
interface UserSettings {
  activeModelId?: string;           // 当前活跃模型的 ModelPoolEntry.id
  modelPool: ModelPoolEntry[];      // 模型池
  systemPrompt: string;             // 系统提示词
  renderThinkingAsMarkdown: boolean; // 思考过程是否渲染为 Markdown
  autoScroll: boolean;              // 自动滚动
  collapseThinkingFinished: boolean; // 完成后折叠思考过程
  gemmaTrimThinkingSpaces: boolean; // Gemma 系列去除思考前导空格
}
```
...
- `GET /api/providers/:id/models` — 获取提供商的模型列表
